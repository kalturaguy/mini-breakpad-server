...
app.post('/post_logs', function (req, res, next) {
    return saver.saveRequest(req, db, function (err, filename) {
        if (err != null) {
            return next(err);
        }
...
app.get("/dumps", function (req, res, next) {
    return res.render('index', {
        title: 'Crash Reports',
        records: db.getAllRecords({"type": undefined})
    });
});
...
app.get("/logs", function (req, res, next) {
    return res.render('index', {
        title: 'Logs Reports',
        records: db.getAllRecords({"type": "log"})
    });
});
...
app.get("/view/:id", function (req, res, next) {
    return db.restoreRecord(req.params.id, function (err, record) {
        ...
        if (record.fields.type==="log") {
            return res.render('view-log', {
                title: 'Log Report',
                record: record
            });
        }
